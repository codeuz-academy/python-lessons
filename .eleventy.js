...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addGlobalData("ga4MeasurementId", ga4MeasurementId);
  ...
  eleventyConfig.addFilter("collectionHasUrl", function (items, candidateUrl) {
    if (!Array.isArray(items) || !candidateUrl) {
      return false;
    }

    return items.some((item) => item && item.url === candidateUrl);
  });
  ...
  eleventyConfig.addCollection("dataStructures", function (collectionApi) {
    return collectionApi.getFilteredByGlob("content/en/data-structures/**/*.md").sort((a, b) => {
      return (a.data.order || 0) - (b.data.order || 0);
    });
  });

  eleventyConfig.addCollection("dataStructuresUz", function (collectionApi) {
    return collectionApi.getFilteredByGlob("content/uz/data-structures/**/*.md").sort((a, b) => {
      return (a.data.order || 0) - (b.data.order || 0);
    });
  });

  eleventyConfig.addCollection("algorithms", function (collectionApi) {
    return collectionApi.getFilteredByGlob("content/en/algorithms/**/*.md").sort((a, b) => {
      return (a.data.order || 0) - (b.data.order || 0);
    });
  });

  eleventyConfig.addCollection("algorithmsUz", function (collectionApi) {
    return collectionApi.getFilteredByGlob("content/uz/algorithms/**/*.md").sort((a, b) => {
      return (a.data.order || 0) - (b.data.order || 0);
    });
  });
